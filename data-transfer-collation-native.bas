...
Sub ProcessWebsitesFromTextFile()
    ...
    For Each website In websiteList
        ...
        If website <> "" Then
            ...
            lastRow = utilSheet.Cells(Rows.Count, "C").End(xlUp).Row

            Set filterRange = utilSheet.Range("A1:Z" & lastRow)
            If Not filterRange Is Nothing Then
                filterRange.AutoFilter Field:=3, Criteria1:=website
                ...
                For i = 2 To sourceLastRow 'Start at row 2 to skip headers
                    ...
                    If newWb.Sheets(1).Cells(i, "D").Value = "Large Tier" Or newWb.Sheets(1).Cells(i, "D").Value = "Large HA" Then
                ...
                saveFileName = "Native - Additional Data Transfer - " & website & ".xlsx"
    ...
    outputFilePath = ThisWorkbook.Path & "\missing-data-report-native.txt" ' Save in the same directory as the Excel file
